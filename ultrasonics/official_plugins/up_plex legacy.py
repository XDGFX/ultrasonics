...
handshake = {
    "name": "plex legacy",
    "description": "(Depricated) Sync playlists to and from Plex Media Server.",
    "type": ["inputs", "outputs"],
    "mode": ["playlists"],
    "version": "0.1",
    "settings": [
        ...
        {"type": "string", "value": "Do you want to check SSL when connecting?"},
        {
            "type": "radio",
            "label": "Check SSL",
            "name": "check_ssl",
...
def run(settings_dict, **kwargs):
    ...
    def fetch_playlist(key):
        ...
        for document in tqdm(
            root.findall("Track"), desc=f"Fetching songs from playlist: {title}"
        ):
    ...
    def remove_prepend(path, invert=False):
        ...
        if not invert:
            if database["plex_prepend"]:
                return (
                    path.replace(database["plex_prepend"], "").lstrip("/").lstrip("\\")
                )
    ...
    if component == "inputs":
        ...
        for key in keys:
            ...
            if re.match(settings_dict["filter"], name, re.IGNORECASE):
                ...
                songs_dict_entry = {"name": name, "id": {}, "songs": playlist}
        ...
        temp_path = os.path.join(database["ultrasonics_prepend"], ".ultrasonics_tmp")
        ...
        if os.path.isdir(temp_path):
            try:
                ...
                raise Exception(
                    "Could not remove temp folder: {temp_path}. Try deleting manually",
                    e,
                )
        ...
        for item in songs_dict:
            ...
            for song in songs:
                ...
                prepend_path_converted = convert_path(database["plex_prepend"])
            ...
            temp_path_plex = os.path.join(database["plex_prepend"], ".ultrasonics_tmp")

            playlist_path_plex = os.path.join(temp_path_plex, item["name"] + ".m3u")

            playlist_path_plex = convert_path(playlist_path_plex, invert=True)

            section_id = re.findall("\[(\d+)\]$", settings_dict["section_id"])[0]

            querystring = urlencode(
                OrderedDict(
                    [
                        ("sectionID", section_id),
                        ("path", playlist_path_plex),
                        ("X-Plex-Token", database["plex_token"]),
                    ]
                )
            )
...
def test(database, **kwargs):
    ...
    if (
        ...
        and database["ultrasonics_prepend"]
        and os.path.isdir(database["ultrasonics_prepend"])
    ):
    ...
    if os.path.isdir(temp_path):
        try:
            ...
            raise Exception(
                f"Could not remove temporary ultrasonics folder {temp_path}. Try removing manually.",
                e,
            )
